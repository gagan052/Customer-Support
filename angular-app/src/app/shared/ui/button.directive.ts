  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        accent: "bg-accent text-accent-foreground hover:bg-accent/90 shadow-md glow-hover",
        success: "bg-success text-success-foreground hover:bg-success/90 shadow-sm",
        warning: "bg-warning text-warning-foreground hover:bg-warning/90 shadow-sm",
        escalate: "bg-escalated text-escalated-foreground hover:bg-escalated/90 shadow-sm",
        glass: "bg-card/50 backdrop-blur-sm border border-border/50 hover:bg-card/80 text-foreground",
        hero: "bg-accent-gradient text-accent-foreground font-semibold shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]",
      },
      size: {
...
export class ButtonDirective implements OnInit, OnChanges {
  ...
  private updateClasses() {
    const classes = cn(buttonVariants({ variant: this.variant, size: this.size }), this.class);
